...
const Cart = () => {
    ...
    const cart = useSelector((state)=>(state.cart))  // {cart} aisa use kiya to state.cart ki jarurat nhi hai.
    const [totalAmount,settotalAmount] = useState(0);

    useEffect(()=>{
        settotalAmount(cart.reduce((acc,curr)=>acc+curr.price,0));
    },[cart])  // cart me jb bhi changes honge tb totalamount nikalo

  return (
    <div>
    
        {/* ab do case ahi ya to acrt empty hogi ya nhi hogi but kese check kare ki cart ke array me kuch items pde hai wkt ki cartslie ke ander hi hum cart ki state maintain kr rhe hai to usi state ko yha use akrenge i=using useselector */}
